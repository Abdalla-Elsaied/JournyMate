...
namespace JourneyMate.Application.ServicesApi.Implementaion
{
    ...
    public class PaymobServices : IPaymobServices
    {
        ...
        public async Task<PaymentResponseDto> InitiatePaymentAsync(PaymentRequestDto request)
        {
                ...
                //put the transaction Id  in booking 
                booking.PaymentIntentId = intentionResult.SpecialReference;
        ...
        private async Task<(string ClientSecret, string SpecialReference)> CreateIntentionAsync(Booking booking, AppUser user, PaymentRequestDto request)
        {
        ...
        private dynamic CreateBillingData(Booking booking, AppUser user)
        {
            return new
            {
                apartment = "N/A",
                first_name = user?.FullName ?? booking.BuyerEmail.Split('@')[0],
                last_name = "Customer",
                street = "N/A",
                building = "N/A",
